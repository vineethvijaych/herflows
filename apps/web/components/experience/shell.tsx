...
function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-2 bottom-2 z-50 md:hidden" aria-label="Primary navigation">
      <div className="flex items-stretch gap-1 rounded-[2rem] border border-white/80 bg-white/95 p-1.5 shadow-xl backdrop-blur-2xl overflow-x-auto scrollbar-hide">
        {mobileRoutes.map((route) => {
          ...
          return (
            <Link
              ...
              href={route.href}
              className={cn(
                'relative flex min-h-16 flex-1 shrink-0 basis-0 flex-col items-center justify-center gap-1 rounded-[1.4rem] px-2 py-1 text-[0.7rem] font-bold transition-all duration-200',
                active
